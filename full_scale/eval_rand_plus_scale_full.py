...
Writes
------
``/cache/results/rand_plus_scale_full_msmarco.json``::

    {
      "method": "rand_plus_scale",
      "n_queries": 6980,
      "n_docs": 8841823,
      "r": 64,
      "use_scale": true,
...
# R is a random orthonormal projection (fixed during training);
# only W_scale was trained.
RELEASE_ROOT = Path(__file__).resolve().parents[1]
LOCAL_CKPT = str(RELEASE_ROOT / "artifacts/checkpoints/ablation_random_plus_scale/best.pt")
...
def run(
    ...
    sweep_ckpt_path = cache / "results" / "sweep_checkpoint_rand_plus_scale.pt"
    ...
    out_path = cache / "results" / f"rand_plus_scale_full_msmarco{_seed_tag}.json"
    ...
    sidecar_path = cache / "results" / f"rand_plus_scale_full_msmarco{_seed_tag}.per_query.pt"
