"""Modal app: trained R, r=128 + scale on full 8.8M MS MARCO dev.

The r=128 trained arm of the scale-held-constant 8.8M comparison. R is the LEARNED
128x128 projection (checkpoint scale_50k_r128_v2, not frozen), W_scale trained,
doc encoded as sign(R.d), query full-precision (asymmetric). Run identically to
``full_scale/eval_rand_plus_scale_full.py`` (same image, pipeline, metrics) so the 8.8M
r=128 block isolates the projection with the scale head held constant — trained vs
random vs identity, all +scale (16 B/tok). Checkpoint: scale_50k_r128_v2.
...
Writes
------
``/cache/results/subbit_r128_full_msmarco.json``::
...
APP_NAME = "subbit-msmarco-eval-subbit-r128"
...
# Trained R, r=128 + scale checkpoint (scale_50k_r128_v2): R is the LEARNED
# 128x128 projection (not frozen), scale head trained, use_scale=true. The r=128
# trained arm of the scale-held-constant 8.8M comparison (trained vs random vs
# identity, all +scale). Do NOT pass --rand-seed (R must stay the checkpoint's
# trained R).
RELEASE_ROOT = Path(__file__).resolve().parents[1]
LOCAL_CKPT = str(RELEASE_ROOT / "artifacts/checkpoints/scale_50k_r128_v2/best.pt")
...
def run(
    ...
    sweep_ckpt_path = cache / "results" / "sweep_checkpoint_subbit_r128.pt"
    ...
    out_path = cache / "results" / f"subbit_r128_full_msmarco{_seed_tag}.json"
    ...
    sidecar_path = cache / "results" / f"subbit_r128_full_msmarco{_seed_tag}.per_query.pt"
