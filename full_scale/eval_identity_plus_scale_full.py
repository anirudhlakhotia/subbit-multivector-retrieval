"""Modal app: identity-R + trained-scale baseline on full 8.8M MS MARCO dev.

The identity analogue of the rand_plus_scale 8.8M baseline: R = [I_64 | 0] (first
64 dims, frozen), W_scale trained with R frozen (5000 steps), doc encoded as
sign(R.d), query full-precision (asymmetric). Run identically to
``full_scale/eval_rand_plus_scale_full.py`` (same image, pipeline, metrics) so the 8.8M
comparison isolates the projection with the scale head held constant — trained vs
random (rand_plus_scale, 0.3724) vs identity. Checkpoint: ablation_identity_plus_scale.
...
``/ckpt/best.pt``           identity-R + trained-scale checkpoint (mounted from local repo)

Writes
------
``/cache/results/identity_plus_scale_full_msmarco.json``::

    {
      "method": "identity_plus_scale",
      "n_queries": 6980,
      "n_docs": 8841823,
      "r": 64,
      "use_scale": true,
...
APP_NAME = "subbit-msmarco-eval-identity-plus-scale"
# Same v2 volume as canonical SubBit eval; read-only here, so parallel runs
# are safe. Each parallel script writes to a distinct output
...
# "Frozen identity R + trained scale head" baseline checkpoint. R = [I_64 | 0]
# (first 64 dims, fixed during training); only W_scale was trained. Mirrors the
# rand_plus_scale recipe exactly (freeze_R + scale, 5000 steps) with init=identity,
# so the 8.8M comparison isolates the projection (identity vs random vs trained),
# scale head held constant. Do NOT pass --rand-seed (R must stay the checkpoint's
# identity, not a regenerated random orthogonal).
RELEASE_ROOT = Path(__file__).resolve().parents[1]
LOCAL_CKPT = str(RELEASE_ROOT / "artifacts/checkpoints/ablation_identity_plus_scale/best.pt")
...
def run(
    doc_chunk: int = 8192,
    query_batch: int = 128,    # 256 OOMs at long-doc chunks; 128 leaves headroom
    top_k: int = 1000,
    ...
    sweep_ckpt_path = cache / "results" / "sweep_checkpoint_identity_plus_scale.pt"
    ...
    out_path = cache / "results" / f"identity_plus_scale_full_msmarco{_seed_tag}.json"
    ...
    sidecar_path = cache / "results" / f"identity_plus_scale_full_msmarco{_seed_tag}.per_query.pt"
