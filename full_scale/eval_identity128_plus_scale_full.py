"""Modal app: IDENTITY R, r=128 + scale on full 8.8M MS MARCO dev.

The r=128 IDENTITY arm of the scale-held-constant 8.8M comparison. R is the frozen
identity (128x128 = no rotation; sign of the raw dims), W_scale trained with R frozen
(checkpoint ablation_identity128_plus_scale), doc encoded as sign(R.d), query
full-precision (asymmetric). Run identically to ``full_scale/eval_rand_plus_scale_full.py``
(same image, pipeline, metrics) so the 8.8M r=128 block isolates the projection with the
scale head held constant — trained vs random vs identity, all +scale (16 B/tok).
Checkpoint: ablation_identity128_plus_scale.
...
Writes
------
``/cache/results/identity128_plus_scale_full_msmarco.json``::

    {
      "method": "identity128_plus_scale",
      "n_queries": 6980,
      "n_docs": 8841823,
      "r": 128,
      "use_scale": true,
...
APP_NAME = "subbit-msmarco-eval-identity128-plus-scale"
# Same v2 volume as canonical SubBit eval — read-only here, so parallel
# runs are safe. Each parallel script writes to a distinct output
...
# Frozen identity R r=128 (no rotation) + trained scale checkpoint
# (ablation_identity128_plus_scale): R frozen identity, only W_scale trained, use_scale=true.
# The r=128 identity arm of the scale-held-constant comparison. Do NOT pass --rand-seed
# (R must stay the checkpoint's frozen identity).
RELEASE_ROOT = Path(__file__).resolve().parents[1]
LOCAL_CKPT = str(RELEASE_ROOT / "artifacts/checkpoints/ablation_identity128_plus_scale/best.pt")
...
def run(
    ...
    sweep_ckpt_path = cache / "results" / "sweep_checkpoint_identity128_plus_scale.pt"
    ...
    out_path = cache / "results" / f"identity128_plus_scale_full_msmarco{_seed_tag}.json"
    ...
    sidecar_path = cache / "results" / f"identity128_plus_scale_full_msmarco{_seed_tag}.per_query.pt"
